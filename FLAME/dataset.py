...
class FaceDataset:
    def __init__(self, dataset_name, shuffle=True):
        self.dataset_name = dataset_name
        file_list = glob.glob(os.path.join(dataset_name, "checkpoint", "*.frame"))
        self.shuffle = shuffle
        self.n_frames = len(glob.glob(os.path.join(dataset_name, "images", "*.png")))
    ...
    def prepare_data(self, reside_image_on_gpu=True, device="cuda"):
        output_list = []
        for ii in tqdm(range(self.n_frames)):
    ...
    def getData(self, id, reside_image_on_gpu=True ,device="cuda"):
        ...
        frame_flame['shape'] = frame_flame['shape'].astype(np.float32)
        frame_flame['exp'] = frame_flame['exp'].astype(np.float32)
        frame_flame['tex'] = frame_flame['tex'].astype(np.float32)
        frame_camera = frame['camera']
        frame_opencv = frame['opencv']

        # NOTE: see this - how they generate the camera
        # print('\n')
        # print(frame_camera['pp'])
        # print(frame_camera['fl'])
        # print(frame_camera['R'])
        # print(frame_camera['t'])
